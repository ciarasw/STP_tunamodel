...
#scenario: Objective: Reduce juvenile catch of yellowfin and bigeye via reduction of FADs


#Scenario setup-
#adjust vuln curve for PS YFT: skew towards larger fish
#reduce Qs for PS: a little bit for YFT, a lot for BET
#increase costs correspondingly
...
for (ii in 1:nfish) {
  ...
  #CHANGED lh1, lh2 for Y and B
  ahatPS = c(30, 400, 400)
  lh1p = c(30, 80, 60) #lower limit of dome (cm)
  lh2p = c(80, 170, 120) #upper lim
...
ggplot(biomass_finalm, aes(Year, Biomass, colour = Species))+
  ...
  geom_point()+
  geom_line()+
  scale_colour_manual(values = colspecies)+
  xlim(c(0,30))+
  labs(title = "SSB (mt)")
ggsave(paste("figures/", scenario, "_SSBovertime.png", sep = ""), height = 7, width = 7, units = "in")
...
CPGm$species = factor(CPGm$species, levels = speciesorder)


ggplot(CPGm, aes(year, catch, colour = gear))+
  theme(axis.title.y = element_blank(),
        legend.position = "bottom")+
  facet_wrap(.~species, scales = "free")+
  geom_point()+
  geom_line()+
  scale_colour_manual(values = colgear)+
  xlim(c(0,10))+
  labs(title = "Catch (mt)")
ggsave(paste("figures/", scenario, "_catchovertime_sp.png", sep = ""), height = 7, width = 7, units = "in")

ggplot(CPGm, aes(year, catch, colour = species))+
  theme(axis.title.y = element_blank(),
        legend.position = "bottom")+
  ...
  labs(title = "Catch (mt)", x = "Year", colour = "Species")
ggsave(paste("figures/", scenario, "_catchovertime_gear.png", sep = ""), height = 7, width = 7, units = "in")
...
PS_costin = 1.1 #cost increase

effort_summary$unitcost = as.vector(c(1, 22000*PS_costin, 2000, 100, 3000, #IN
                                      22000*PS_costin, 2000, 100, #PH
                                      1, 22000*PS_costin, 100, #VN
                                      1, 22000*PS_costin #other
))
...
effort_summary$unitcost

# econ_time = CPGm[, order(year, )]

head(CPGm, 15) #rmbr only goes to year 49
dim(CPGm)
CPGm$unitprice = as.vector(
  (c(
    rep(c(0, 8000, 11000), 49), #INLL
    rep(c(1600, 2300, 2600), 49), #INPS
    rep(c(1850, 7000, 10000), 49), #INHL
    rep(c( 1300, 2000, 2300), 49), #INSSS
    rep(c(1850, 2550, 2850), 49), #INPL
    rep(c(1600, 2300, 2600), 49), #PHPS
    rep(c(1850, 7000, 10000), 49), #PHHL
    rep(c(1300, 2000, 2300), 49), #PHSSS
    rep(c(0, 8000, 11000), 49), #VNLL
    rep(c(1600, 2300, 2600), 49), #VNPS
    rep(c(1300, 2000, 2300), 49), #VNSSS
    rep(c(0, 8000, 11000), 49), #otherLL
    rep(c(1600, 2300, 2600), 49) #otherPS
)))

CPGm$exvesselprice = CPGm$catch * CPGm$unitprice
head(CPGm, 15)
tail(CPGm, 15)



prices_gear_time = CPGm %>% group_by(gear, year) %>% summarise(exvesselprice = sum(exvesselprice))

# View(prices_gear_time)
